...
#[derive(Debug, Clone)]
/// Configuration for parameters that determine how your inbound liquidity service runs.
///
/// Fees you earn for channel open
/// Fees you earn for routing payments
/// Min/Max payment and channel sizes
pub struct LiquidityProviderConfig {
	/// Used to verify offers were generated by you. Should be same value across restarts.
...
impl Default for LiquidityProviderConfig {
	fn default() -> Self {
		Self {
			promise_secret: [0; 32],
			funding_tx_fee_multiplier: 2.0,
			funding_tx_fee_multiplier_source_rate: ConfirmationTarget::NonAnchorChannelFee,
			minimum_min_fee_msat: 10_000_000,
			// 1% fee
			proportional_fee: 10_000,
			fee_valid_for_minutes: 10,
			// ~45 days
			min_lifetime_blocks: 6480,
			// ~2 weeks
			max_client_to_self_delay_blocks: 2016,
			min_payment_size_buffer_msats: 1000,
			max_payment_size_msats: 1_000_000_000,
			// ~12 hours
			client_invoice_cltv_expiry_delta: 72,
			channel_size_factor: 2.0,
			min_channel_size_sats: 100_000,
			forwarding_fee_base_msat: 0,
			// 0.4% fee
			forwarding_fee_proportional_millionths: 4000,
		}
	}
}
...
impl NodeBuilder {
	...
	pub fn set_liquidity_provider_lsps2(&mut self, config: LiquidityProviderConfig) -> &mut Self {
		self.config.lsps2_liquidity_provider_config = Some(config);
		self
	}
...
fn build_with_store_internal(
	...
	if config.lsps2_liquidity_provider_config.is_some() {
	...
	if let Some(liquidity_provider_config) = &config.lsps2_liquidity_provider_config {
		let liquidity_manager = Arc::new(LiquidityManager::new(
			Arc::clone(&keys_manager),
			Arc::clone(&channel_manager),
			Some(Arc::clone(&tx_sync)),
			...
			Some(LiquidityServiceConfig {
				lsps2_service_config: Some(LSPS2ServiceConfig {
					promise_secret: liquidity_provider_config.promise_secret,
				}),
